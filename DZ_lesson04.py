...
def download_image(url):
    ...
    print(f"Загрузка файлов {filename} in {end_time:.2f} секунд")
    ...
    with open(filename, "wb") as f:
        for chunk in response.iter_content(chunk_size=1024):
            if chunk:
                f.write(chunk)
    end_time = time.time() - start_time
    print(f"Загрузка {filename} in {end_time:.2f} секунд")
...
def download_images_threading(urls):
    ...
    end_time = time.time() - start_time
    print(f"общее время работы функции threading: {end_time:.2f} секунд")
...
def download_images_multiprocessing(urls):
    ...
    end_time = time.time() - start_time
    print(f"Общее время работы multiprocessing: {end_time:.2f} секунд")
